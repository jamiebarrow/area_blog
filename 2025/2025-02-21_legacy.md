...
Using measures to guide the growth of a garden is similar to using observability and metrics to optimize software performance.
...
A farmer protects his livestock from being hunted by foxes.
A gardener tries to keep out infestations of bugs that destroy his crops.
...
Grapes and other creeping vines are beautiful to look at, but if you don't take care they can creep and grow out of hand, suffocating other plants, and even damaging walls of buildings they cling to. They need regular pruning to keep the balance and avoid a tangled mess.

This is the same with code in software systems. There are tools that can help manage the growing software forests:

- Cyclomatic complexity to minimize complexity
- Unit test code coverage reports to ensure a safety net
- Static analysis with tools like EditorConfig, Checkstyle, SonarQube, and eslint to keep to standards and consistency

An interesting tool is [CodeCharta](https://codecharta.com/docs/analysis/metrics), which visualizes your codebase as a city map to help you spot problematic areas, such as code churn to highlight problematic areas due to constant change, or the opposite: knowledge gaps in the team due to lack of exposure. 
